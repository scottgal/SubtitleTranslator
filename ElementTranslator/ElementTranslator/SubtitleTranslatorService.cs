...
public class SubtitleTranslatorService
{
    ...
    private readonly ParallelOptions _languageParalellOptions =
        new() { MaxDegreeOfParallelism = Environment.ProcessorCount };

    private readonly ParallelOptions _lineParalellOptions;
    private ProgressTask _totalTask;

    private int ExecutorCount;

    private int MaxLangName;
    ...
    public SubtitleTranslatorService(LibreTranslateService libreTranslateService, TranslateConfig translateConfig)
    {
        ...
        _lineParalellOptions =
            new ParallelOptions
            {
                MaxDegreeOfParallelism = Environment.ProcessorCount / _languageParalellOptions.MaxDegreeOfParallelism
            };
    }
        ...
        HttpClient httpClient)
    {
        ...
        await AnsiConsole.Progress()
            ...
            .StartAsync(async ctx =>
            {
                ...
                {
                    var task = ctx.AddTask(TaskDescription(null, isInitial: true, description: item.name),
        ...
        Languages languageCode, List<SubtitleItem> items, ProgressTask task)
    {
        ...
        await Parallel.ForEachAsync(procItems, _lineParalellOptions,
            async (item, t) =>
            {
                ...
                task.Description = TaskDescription(task, languageCode.name);


                if (task.Value % 100 == 0) await WriteTempFile(languageCode, outItems);
                outItems.Add(newItem);
            });
        await WriteTempFile(languageCode, outItems);
        File.Move(GetOutputFileName(languageCode.code, true), GetOutputFileName(languageCode.code));
        AnsiConsole.MarkupLine("[bold]Finished[/]: [green]{0}[/]", languageCode.name);
        task.StopTask();
    }

    private string TaskDescription(ProgressTask? task, string description, bool isTitle = false, double maxValue = 0,
        bool isInitial = false)
    {
        ...
        var rowStyle = new Style().Decoration(Decoration.Bold)
            .Foreground(isTitle ? Color.Red : isInitial ? Color.Orange1 : Color.Green).ToMarkup();
