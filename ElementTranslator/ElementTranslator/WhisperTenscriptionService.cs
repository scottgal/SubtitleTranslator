...
public class WhisperTenscriptionService
{
    ...
    public async Task<WhisperDetectLanguage> DetectLanguage(HttpClient whisperHttpClient, string mp3FilePath)
    {
        ...
        await AnsiConsole.Status()
            ...
            .StartAsync("[yellow]Beginning subtitle for file[/]" + Path.GetFileName(mp3FilePath), async ctx =>
            {
                ...
                await using var stream = File.OpenRead(mp3FilePath);
                ...
                try
                {
                    ...
                    if (!resp.IsSuccessStatusCode)
                    {
                        AnsiConsole.MarkupLine(
                            $"[red]Error[/] [white bold] {resp.StatusCode}[/] [red] for file.[/] {mp3FilePath}");
                        return;
                    }

                    outStr = JsonSerializer.Deserialize(await resp.Content.ReadAsStringAsync(),
                        TranslatorJsonContext.Default.WhisperDetectLanguage);
                    var elapsed = sw.ElapsedMilliseconds - startTimestamp;
                    AnsiConsole.MarkupLine(
                        $"[green]Detected  language[/] [white bold] {outStr}[/][green] for file in {TimeSpan.FromMilliseconds(elapsed).TotalMinutes} minutes.[/] {mp3FilePath}");
                }
                catch (Exception)
                {
                    //TODO: LOGGING.
    ...
    public async Task<bool> TranscribeVideoFile(HttpClient whisperHttpClient, TranslateConfig config)
    {
        ...
        await AnsiConsole.Status()
            ...
            {
                ctx.Status(
                    $"Beginning Subtitle Transcription for file, this will take a few minutes. {config.Mp3Path}");
                ctx.Refresh();
                ...
                try
                {
                    ...
                    var output = config.SubtitleFilePath;
                    await File.WriteAllBytesAsync(output, await response.Content.ReadAsByteArrayAsync());
                    var elapsed = sw.ElapsedMilliseconds - startTimestamp;
                    AnsiConsole.MarkupLine(
                        $"[green]Cpmpleted Subtitle Transcription for file in {TimeSpan.FromMilliseconds(elapsed).TotalMinutes} minutes.[/] {config.Mp3Path}");
                }
                catch (Exception e)
                {
    ...
    public async Task<(bool success, string filePath)> ExtractAudioFromVideoFile(string videoFilePath,
        string mp3FilePath)
    {
        var startTimestamp = sw.ElapsedMilliseconds;
        ...
        await AnsiConsole.Status()
            ...
            .StartAsync("[yellow]Beginning mp3 extraction for file[/]" + Path.GetFileName(videoFilePath), async ctx =>
            {
                ...
                {
                    ctx.Status(
                        $"{args.Duration}/{args.TotalLength}   {Math.Round((double)args.Percent)}% {videoFilePath}");
                };
